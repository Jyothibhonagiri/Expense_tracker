...
function Navbar() {
    ...
    const getUserProfile = async () => {
        ...
        setUser(userInfo);
        if (userInfo) {
            fetch("https://localhost:7000/auth/profile/" + userInfo.email, {
                headers: {
                        ...
                        <li className="nav-item dropdown">
                            <Link className="nav-link" role="button" onClick={dropDownToggle}>
                                <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: '#ddd' }}>
                                    <img src={`https://localhost:7000/uploads/${profile?.profilepic}`} style={{ width: '100%', height: '100%', borderRadius: '50%' }} alt="Profile" />
                                </div>
                            </Link>
                            {isToggled && (
