...
function Users() {
    ...
    const getAllUsers = async () => {
        fetch(`https://localhost:7000/auth/users`).then((res) => {
            return res.json();
        }).then((result) => {
            console.log(result)
            setUsers(result)
    ...
    const accountActivateDeactiate = (e, id, active) => {
        ...
        fetch("https://localhost:7000/auth/activate_deactivate", { method: "PUT", headers: { "Content-type": 'Application/Json' }, body: JSON.stringify({ id: id, active: activeFlag }) }).then((res) => {
            return res.json();
        }).then((result) => {
            getAllUsers();
        })
